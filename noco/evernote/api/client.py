...
import noco.evernote.edam.userstore.UserStore as UserStore
import noco.evernote.edam.notestore.NoteStore as NoteStore
import noco.evernote.edam.userstore.constants as UserStoreConstants

import noco.thrift.protocol.TBinaryProtocol as TBinaryProtocol
import noco.thrift.transport.THttpClient as THttpClient
...
class EvernoteClient(object):
    ...
    def get_note_store(self, user_store=None):
        if not user_store:
    ...
    def find_note(self, keyword, user_store=None):
        if not user_store:
            user_store = self.get_user_store()
        note_store_uri = user_store.getNoteStoreUrl()
        note_store = Store(self.token, NoteStore.Client, note_store_uri)
        spec = NoteStore.NotesMetadataResultSpec(
            includeTitle=True, includeAttributes=True
        )
        noteList = note_store.findNotesMetadata(
            NoteStore.NoteFilter(words=keyword), 0, 100, spec
        )
